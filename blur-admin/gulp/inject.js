...
gulp.task('inject', ['scripts', 'scriptsAuth', 'styles', 'injectAuth', 'inject404', 'injectGenericError', 'copyVendorImages'], function () {
    var injectStyles = gulp.src([
        path.join(conf.paths.tmp, '/serve/app/main.css'),
        path.join('!' + conf.paths.tmp, '/serve/app/vendor.css')
    ], {read: false});

    var injectStylesAuth = gulp.src([
    ...
    var injectScriptsAuth = gulp.src([
    ...
    gulp.src(path.join(conf.paths.src, '/auth.html'))
        .pipe($.inject(injectStylesAuth, injectOptions))
        .pipe($.inject(injectScriptsAuth, injectOptions))
...
gulp.task('injectGenericError', ['styles404'], function () {
    return injectAlone({
        css: [path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'), path.join(conf.paths.tmp, '/serve/app/404.css')],
        paths: path.join(conf.paths.src, '/error.html')
    })
});
