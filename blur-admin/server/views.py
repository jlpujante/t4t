import logging
from flask import Blueprint, redirect, make_response, render_template, send_from_directory, request
...
def home():
    if not app_redis_session.is_valid_current_session():
        return redirect("%s?origin=%s" % (tracker_app.APP_LOGIN_URL, request.path))
    # print app.bundles
